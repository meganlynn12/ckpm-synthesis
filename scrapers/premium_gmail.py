...
Tier detection (dynamic, per email):
  ...
  nytdirect@nytimes.com + "david french" in From → "longform"
  email@theatlantic.com + known subject → "newsletter"
  email@theatlantic.com + other         → "longform"
...
URLs are extracted from raw HTML before stripping and passed to the
summarizer so newsletter item links are preserved even when they are
...
def _detect_tier(source: dict, subject: str, from_header: str = "") -> str:
    """
    Determine the correct tier for an email based on source, subject, and From header.

    Priority order:
    1. Breaking news sender → always "breaking"
    2. NYT + "david french" in From header → "longform"
    3. Atlantic + known newsletter subject → "newsletter"
    4. Atlantic + unknown subject → "longform"
...
def _get_raw_html(payload: dict) -> str:
    """Extract raw HTML from payload without stripping tags."""
...
def _extract_urls_from_html(html: str) -> list[str]:
    """
    Extract all href URLs from raw HTML before tag stripping.
    Filters out unsubscribe, mailto, anchor, and tracking pixel URLs.
    ...
    for href in hrefs:
        ...
        if not href.startswith("http"):
            continue
        skip_patterns = [
            "unsubscribe", "mailto", "optout", "opt-out",
            "preferences", "manage-preferences", "email-settings",
            "pixel", "track", "open.php", "beacon",
        ]
